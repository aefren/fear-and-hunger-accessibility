 ...
 * offset, volume = distance). Pings every two seconds, or once a second within a few tiles.
 ...
 *
 * @param Far Interval
 * @desc Frames between pings for a distant corpse. 120 frames = 2 seconds.
 * @type number
 * @default 120
 *
 * @param Near Interval
 * @desc Frames between pings for a near corpse (<= Near Threshold). 60 = one second.
 * @type number
 * @default 60
 *
 ...
 * Cadence is per corpse: every two seconds normally, once a second once the corpse
...
(function () {
    ...
    function ping(dx, dy, dist) {
        ...
        // Volume: louder near fading to quiet far by ~30 tiles. Max 15 so
        // corpses sit well under enemies/doors in the mix.
        var d = Math.min(dist, 30);
        var volume = Math.round(15 - (d / 30) * 7);
