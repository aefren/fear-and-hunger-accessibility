 ...
 * Pings every two seconds, or once a second within a few tiles. No toggle. Sibling of
 ...
 *
 * @param Far Interval
 * @desc Frames between pings for a distant altar. 120 frames = 2 seconds.
 * @type number
 * @default 120
 *
 * @param Near Interval
 * @desc Frames between pings for a near altar (<= Near Threshold). 60 = one second.
 * @type number
 * @default 60
 *
 ...
 * Cadence is per altar: every two seconds normally, once a second once it is within
...
(function () {
    ...
    function ping(dx, dy, dist) {
        ...
        // above doors/containers in the mix (max 40) -- they are rare, important
        // landmarks -- but well under enemies (max 60).
        var d = Math.min(dist, 30);
        var volume = Math.round(40 - (d / 30) * 27);
